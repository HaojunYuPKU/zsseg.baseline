...
def register_all_ade20k_full_mini(root):
    root = os.path.join(root, "ADE20K_2021_17_01")
    meta = _get_ade20k_full_meta()
    for name, dirname in [("train", "training"), ("val", "validation")]:
        image_dir = os.path.join(root, "images_detectron2_mini", dirname)
        gt_dir = os.path.join(root, "annotations_detectron2_mini", dirname)
        name = f"ade20k_full_mini_sem_seg_{name}"
        DatasetCatalog.register(
            name, lambda x=image_dir, y=gt_dir: load_sem_seg(y, x, gt_ext="tif", image_ext="jpg")
        )
        MetadataCatalog.get(name).set(
...
register_all_ade20k_full_mini(_root)
