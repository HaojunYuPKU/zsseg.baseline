...
def _get_pascal_context_stuff_meta(cat_list):
...
def register_pascal_context_59(root):
    ...
    for name, image_dirname, sem_seg_dirname in [
        ...
        name = f"pascal_context_{name}_sem_seg"
...
def register_pascal_context_459(root):
    root = os.path.join(root, "pascal_context")
    meta = _get_pascal_context_stuff_meta(PASCAL_CONTEXT_459_CATEGORIES)
    for name, image_dirname, sem_seg_dirname in [
        ("train", "train/image", "train/label"),
        ("val", "val/image", "val/label"),
    ]:
        image_dir = os.path.join(root, image_dirname)
        gt_dir = os.path.join(root, sem_seg_dirname)
        name = f"pascal_context_full_{name}_sem_seg"
        DatasetCatalog.register(
            name,
            lambda x=image_dir, y=gt_dir: load_sem_seg(
                y, x, gt_ext="png", image_ext="jpg"
